            ...
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0"
                    ...
                    :key="item1.id"
                    :class="{active:item1.isChecked==='1'}"
                    @click="changeActive(item,item1.id)">{{ item1.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.trim.number="skuNum" @change="changeSkuNum">
...
export default {
  ...
  methods: {
    // 改变属性高亮
    changeActive (arr, id2) {
      arr.spuSaleAttrValueList.forEach((item) => {
        item.isChecked = item.id === id2 ? '1' : '0'
      })
    },
    // 输入框输入商品数目
    changeSkuNum () {
    ...
    async addShopCar () {
      try {
        await this.$store.dispatch('detail/addShopCar', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        this.computeAttrList()
        sessionStorage.setItem('SELECTEDATTRLIST', JSON.stringify(this.selectedAttrList))
    ...
    computeAttrList () {
      this.spuSaleAttrList.forEach((item) => {
        item.spuSaleAttrValueList.some((item1) => {
          if (item1.isChecked === '1') {
            this.selectedAttrList.push(item1.saleAttrValueName)
            return true
          }
          return false
        })
