  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in address" :key="item.id">
        ...
        <p @click="changedefaultAddress(item)">
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="item in detailArrayList" :key="item.skuId">
            ...
            <li>
              <p style="width: 350px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">{{item.skuName}}</p>
              <h4>7天无理由退货</h4>
            </li>
            <li>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b><i>{{goodsListInfo.totalNum}}</i> 件商品，总商品金额</b>
          <span>¥ {{goodsListInfo.totalAmount}}</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">应付金额: <span>¥ {{goodsListInfo.totalAmount}}</span></div>
...
export default {
  ...
  computed: {
    ...mapState('trade', ['address', 'goodsListInfo', 'orderId']),
  ...
  mounted () {
    try {
      ...
      this.$store.dispatch('trade/getGoodsListInfo')
    } catch (e) {}
  },
  methods: {
    // 改变默认地址
    changedefaultAddress (val) {
      ...
      this.address.forEach(item => item.isDefault = '0')
      val.isDefault = '1'
    },
    // 提交订单
    async submitOrder () {
      const { tradeNo } = this.goodsListInfo
      const data = {
      ...
      try {
        await this.$store.dispatch('trade/submitOrder', { tradeNo, data })
